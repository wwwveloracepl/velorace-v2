...
import { safeStartlistUploadFileName, startlistGroupBlobPrefix } from '@/lib/startlists'
...
export async function POST(req: NextRequest, ctx: { params: { id: string } | Promise<{ id: string }> }) {
  ...
  if (categoryIds.length < 2) {
    return NextResponse.json(
      { ok: false, message: 'Wybierz co najmniej dwie kategorie do jednej listy.' },
      { status: 400 },
    )
  }
  ...
  // Zachowaj kolejność display z bazy
  found.sort((a, b) => {
  ...
  const takenRows = await sql`
    ...
    JOIN race_startlist_groups g ON g.id = gc.group_id
  ...
  try {
    const inserted = await sql`
      INSERT INTO race_startlist_groups (race_id, label)
    ...
    for (const cat of found) {
      await sql`
        INSERT INTO race_startlist_group_categories (group_id, category_id)
        VALUES (${groupId}::uuid, ${cat.id}::uuid)
      `
    }

    const folderPrefix = startlistGroupBlobPrefix(raceCtx.slug, raceCtx.raceYear, groupId)
    ...
    console.error('[startlist-groups/upload]', e)
    return NextResponse.json({ ok: false, message: 'Nie udało się wgrać listy dla grupy.' }, { status: 500 })
  }
}
...
export async function DELETE(req: NextRequest, ctx: { params: { id: string } | Promise<{ id: string }> }) {
  ...
  try {
    ...
    await sql`DELETE FROM race_startlist_groups WHERE id = ${groupId}::uuid AND race_id = ${raceId}::uuid`

    return NextResponse.json({ ok: true })
  } catch (e) {
    console.error('[startlist-groups/upload DELETE]', e)
    return NextResponse.json({ ok: false, message: 'Nie udało się usunąć listy grupy.' }, { status: 500 })
  }
}
