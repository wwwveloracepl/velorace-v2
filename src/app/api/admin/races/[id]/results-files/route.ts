...
function parseFolderFile(
  ...
  // wyscigi_YYYY/{slug}/{kategorie|fale}/{id}/{file}
  if (parts.length < 5) return null
  if (parts[2] !== folderName) return null
  const id = parts[3]
...
export async function GET(req: NextRequest, ctx: { params: { id: string } | Promise<{ id: string }> }) {
  ...
  try {
    const raceMeta = await sql`
      SELECT
        COALESCE(results_combined_file_url, '') AS url,
        COALESCE(results_combined_file_name, '') AS file_name,
        COALESCE(results_combined_uploaded_at::text, '') AS uploaded_at
      FROM races
    ...
    const combinedRows = await sql`
      ...
      FROM race_results_combined_files g
    ...
    try {
      const catBlobs = await listAllBlobsWithPrefix(
        flexibleResultsCategoriesForRaceBlobPrefix(raceCtx.slug, raceCtx.raceYear),
      )
      for (const blob of catBlobs) {
        ...
        categoryFileNames[parsed.id] = safeFlexibleResultUploadFileName(parsed.fileName)
      }
    } catch {
    ...
    try {
      const waveBlobs = await listAllBlobsWithPrefix(
        flexibleResultsWavesForRaceBlobPrefix(raceCtx.slug, raceCtx.raceYear),
      )
      ...
      for (const blob of waveBlobs) {
        ...
        byWave.set(parsed.id, {
          ...
          fileName: safeFlexibleResultUploadFileName(parsed.fileName),
        })
      }
    ...
    console.error('[admin/races/[id]/results-files GET]', e)
    return NextResponse.json({ ok: false, message: 'Nie udało się pobrać wyników.' }, { status: 500 })
  }
}
