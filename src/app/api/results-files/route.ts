  ...
  try {
    ...
    try {
      const catBlobs = await listAllBlobsWithPrefix(
        flexibleResultsCategoriesForRaceBlobPrefix(ctx.slug, ctx.raceYear),
      )
      for (const blob of catBlobs) {
        const parsed = parseFolderFile(blob.pathname, 'kategorie')
        if (!parsed) continue
        urls[parsed.id] = blob.downloadUrl || blob.url
        fileNames[parsed.id] = safeFlexibleResultUploadFileName(parsed.fileName)
      }
    } catch {
      // ignore
    }
    ...
    if (sql) {
      ...
      try {
        const waveBlobs = await listAllBlobsWithPrefix(
          flexibleResultsWavesForRaceBlobPrefix(ctx.slug, ctx.raceYear),
        )
      ...
      try {
        ...
        for (const g of groupRows as { id: string; label: string; file_url: string; file_name: string }[]) {
          const gc = await sql`
            ...
            FROM race_results_group_categories
        ...
        // tabela może jeszcze nie istnieć przed migracją
      }
    }
